...
function createMockFirebase() {
    // Create mock Firebase objects for offline operation
...
function syncLocalDataWithFirebase() {
    ...
    if (Object.keys(localTasks).length > 0) {
        console.log("Syncing local tasks with Firebase...");
...
async function teamLogin(email, password) {
    try {
        ...
        // Check hardcoded credentials (works offline)
...
async function adminLogin(email, password) {
    try {
        ...
        // Check hardcoded admin credentials (works offline)
        if (adminCredentials[lowerEmail]) {
        ...
        // Check if it's a team member trying to login as admin
        if (teamMembers[lowerEmail]) {
            showNotification("Please use Team Login page for team accounts", "error");
            return false;
        }
        
        // If online, check Firebase for additional admins
        if (isFirebaseInitialized && db) {
            try {
                const snapshot = await db.ref(`employees/${lowerEmail}`).once('value');
                const employee = snapshot.val();
...
function loginSuccess(name, email, isAdminFlag, isSystemAdminFlag, userData) {
    ...
    // If Firebase is available, setup listeners
    if (isFirebaseInitialized) {
        setupFirebaseRealtimeListeners();
    }
    ...
    try {
        ...
        if (isFirebaseInitialized && db) {
            try {
                ...
                return true; // Return true because task was saved locally
            }
        } else {
            showNotification("Task saved locally. Will sync when online.", "warning");
            return true;
...
async function getAllEmployees() {
    try {
        // Always return from local storage first
...
async function updateEmployee(email, updates) {
    try {
        if (!isSystemAdmin) {
            showNotification("Only system admin can update employees", "error");
            return false;
        }
        
        if (email === 'chhabi@landscape.com' && updates.role && updates.role !== 'system_admin') {
            showNotification("Cannot change system admin's role", "error");
            return false;
        }
        ...
        // Try Firebase first
        if (isFirebaseInitialized && db) {
            await db.ref(`employees/${email}`).update(finalUpdates);
        }
        
        // Always update local storage
        if (employees[email]) {
            employees[email] = { ...employees[email], ...finalUpdates };
            localStorage.setItem('greenfield_employees', JSON.stringify(employees));
        }
        
        showNotification("Employee updated!" + (isFirebaseInitialized ? " Synced to all devices." : " Saved locally."), "success");
...
function logout() {
    ...
    // Clear session storage but keep tasks
    sessionStorage.removeItem('greenfield_user');
...
function getEmployeeName(email) {
    ...
    // Check employees from local storage
    if (employees[email]) {
